...
with st.sidebar:
    ...
    if os.path.exists(MAPPING_PATH):
        st.markdown('<span class="status-ok">✅ 결과_format.xlsx 로드됨</span>', unsafe_allow_html=True)
    else:
        st.markdown('<span class="status-warn">⚠️ data/결과_format.xlsx 없음</span>', unsafe_allow_html=True)
    st.markdown("---")
...
with c1:
    ...
    if total_stock > 0:
        ...
        fig = px.pie(pie_df, names='구분', values='수량',
                     color_discrete_sequence=['#B45309','#D97706'], hole=0.45)
...
with c3:
    ...
    if not top.empty:
        ...
        fig.update_layout(**LAYOUT, margin=MARGIN_SM,
                          xaxis=dict(gridcolor='#E8E4DC'), yaxis=dict(gridcolor='#E8E4DC'),
